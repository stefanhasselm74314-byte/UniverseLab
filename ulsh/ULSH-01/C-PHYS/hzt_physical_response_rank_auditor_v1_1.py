#!/usr/bin/env python3
"""ULSH-01/C-PHYS physical response-rank auditor v1.1.

Input: R(h), R(h/2), R(h/4) as CSV and frozen normalization scales as JSON.
Audit matrix: J = Sy^{-1} R Sc.
This audits only the response-rank gate; it does not release K1-D or K1-E.
"""
...
def main():
    ...
    eps_step=float(np.linalg.norm(Jh2-Jh4,2)); eps_solver=max(0.0,float(a.solver_refinement_epsilon)); epsJ=eps_step+eps_solver
    rel_change=float(eps_step/max(1.0,np.linalg.norm(Jh4,2)))
    d1=float(np.linalg.norm(Jh-Jh2,2)); d2=float(np.linalg.norm(Jh2-Jh4,2)); rich=d1/d2 if d2>0 else (4.0 if d1==0 else math.inf)
    sigmax=float(s4[0]); sigmin=float(s4[-1]); threshold=a.formal_rel_tol*sigmax; rank=int(np.sum(s4>threshold))
    ...
    direction_ok=ang<=a.angle_max_deg
    ...
    if not a.branch_ok:
        ...
        verdict="NUMERICAL_OR_BRANCH_RESOLUTION_INSUFFICIENT"; reason="sigma_min is not separated from empirical Jacobian uncertainty."
    elif not cond_ok:
        verdict="NUMERICAL_OR_BRANCH_RESOLUTION_INSUFFICIENT"; reason="Formal full rank but conditioning exceeds guardrail."
    elif not direction_ok:
        verdict="NUMERICAL_OR_BRANCH_RESOLUTION_INSUFFICIENT"; reason="Smallest response direction is unstable under refinement."
    else:
        verdict="PHYSICAL_RESPONSE_RANK_4_CONFIRMED"; reason="ULSH-01 numerical response-rank conditions passed."

    out={
      "schema":"ulsh01.cphys.response-rank.audit.v1.1","status":"NUMERICAL_AUDIT_ONLY",
      ...
      "svd":{"singular_values":s4,"formal_rank":rank,"formal_threshold":threshold,"condition_number":k4,"condition_number_max":a.cond_max,"sigma_min_over_epsilon_J":sep,"required_q":a.q,"smallest_direction_angle_deg":ang,"angle_max_deg":a.angle_max_deg,"left_nullspace":U4[:,rank:]},
      "gates":{"branch_ok":a.branch_ok,"derivative_convergence_ok":converged,"rank4_uncertainty_separated":separated,"conditioning_ok":cond_ok,"smallest_direction_stable":direction_ok},
