...
The public/sitemap German comparison route is `compare-safe.html`, while the language matrix and `compare-en.html` declared `compare-desktop.html` as the German alternate. This produced an inconsistent canonical route model.
...
`compare-desktop.html` remains a German presentation alias that maps to the same English entry.

### SEO-LANG-003 — Incomplete sitemap hreflang reciprocity

The sitemap listed the curated DE/EN URLs but only the portal entry carried reciprocal language alternates. Search engines therefore lacked a complete machine-readable pairing for the remaining curated pages.
...
The governed bootstrap now loads the language switcher directly. The existing root-scope service worker now injects both the language switcher and print/export utility into subsequent same-origin HTML navigations when absent.
...
This is not solved by pretending service-worker scope is retroactive. Full elimination requires either rewriting every legacy HTML entry point or moving to a build/template system that emits the language bootstrap into every page.
...
This is a content-coverage gap, not an SEO error, provided they are not falsely advertised as curated English pages.
...
If wording diverges between German and English, the German research source controls the scientific interpretation until the translation is re-reviewed.
...
Add explicit absolute `canonical`, reciprocal `hreflang` and `x-default` elements to both members of every curated pair, not just the sitemap. This should be generated from the route registry to prevent drift.
...
The current route map exists in both JavaScript and the machine-readable registry. Migrate the router to load or generate from the registry during a build step. Runtime-fetching the registry is possible but build-time generation is more robust for first paint and offline-safe static hosting.

### P4 — Finish curated English coverage

Decide whether `about`, `journey`, `emergence` and `universe3d` need curated English versions. Until then, leave them explicitly in automatic-translation fallback status.
