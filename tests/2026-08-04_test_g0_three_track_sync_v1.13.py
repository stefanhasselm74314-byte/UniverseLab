#!/usr/bin/env python3
"""Regression tests for G0 v1.13 Background-3C2 canonical state."""
...
TOOL = ROOT / "tools/2026-08-04_validate_g0_three_track_sync_v1.13.py"
SPEC = importlib.util.spec_from_file_location("g0_v1_13", TOOL)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("unable to import G0 v1.13 validator")
...
def test_repository_state_passes() -> None:
    ...
    assert result["dual_backend_audit"] == "PASS_DUAL_BACKEND_CONTROL_AUDIT_NO_NONLINEAR_EXECUTION"
    assert result["primary_newton_calls"] == 0
    assert result["shooting_jacobian_calls"] == 0
    assert result["decision"] == "UL-DEC-0028"
...
def test_manifest_rejects_root_overclaim() -> None:
    manifest = MOD.load_json("project-manifest.json")
    changed = copy.deepcopy(manifest)
    changed["c_phys_background_3c"]["independent_shooting_root_calls"] = 1
    expect_failure(lambda: MOD.validate_manifest(changed), "shooting root")


def test_manifest_rejects_target_solve_overclaim() -> None:
    manifest = MOD.load_json("project-manifest.json")
    changed = copy.deepcopy(manifest)
    changed["c_phys_background_3c"]["target_model_solves"] = 1
    expect_failure(lambda: MOD.validate_manifest(changed), "target solve")
...
def main() -> int:
    ...
    test_manifest_rejects_root_overclaim()
    test_manifest_rejects_target_solve_overclaim()
    test_checkpoint_rejects_background_overclaim()
    test_manifest_rejects_next_block_drift()
    print("PASS: G0 v1.13 canonical regression tests")
    return 0
