...
def main() -> None:
    ...
    # PR #236 regression: use genuinely operative-shaped payloads so the test
    # exercises the former positive authorization shape rather than synthetic
    # non-authorizing fixtures.
    ...
    # Ad-hoc, implied-consent or unreferenced fields cannot substitute for an
    # independently verifiable owner-ratification binding.
    forged_payloads = []
    for base in (operative_decision, operative_grant):
        for forged_fields in (
            ...
            forged_payloads.append((
                "AUTHORIZATION_DECISION" if "decision_status" in forged["payload"] else "SINGLE_USE_GRANT",
                forged,
            ))
    for artifact_type, forged in forged_payloads:
        expect_error(
            "OWNER_RATIFICATION_NOT_VERIFIABLE",
            lambda artifact_type=artifact_type, forged=forged: V.verify_envelope(
                operative_contract,
                operative_root,
                forged,
                expected_artifact_type=artifact_type,
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        operative_envelope_path = root / "operative-envelope.json"
        ...
        operative_envelope_path.write_text(json.dumps(operative_decision), encoding="utf-8")
        ...
        operative_failure = subprocess.run([
            ...
            "--envelope", str(operative_envelope_path),
            "--expected-artifact-type", "AUTHORIZATION_DECISION",
            "--now-utc", "2026-09-02T01:00:00Z",
        ], check=False, capture_output=True, text=True)
    ...
    print("ULSH-01 WP2 authority/signature provenance v0.2 bridge QA: PASS")
