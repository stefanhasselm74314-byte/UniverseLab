(()=>{
  ...
  const init=()=>{
    ...
    ).forEach(link=>{link.href='./emergence.html?v=07';});

    const pages=[
      ['./','⌂','Portal','portal'],
      ['./emergence.html?v=07','▦','UniverseLab','emergence'],
    ...
    const statuses={
      ...
      guide:['#72cfff','Lern- und Referenzsystem'],
      about:['#8d7cff','Projektinformation']
    };
    const status=statuses[pageKey]||['#70dfb3','ΛCDM / Analyse'];

    const guideAnchors={
      portal:'start',emergence:'emergence',journey:'journey',observatory:'observatory',
      compare:'compare',hyperlab:'hyperlab',universe3d:'universe3d',validation:'validation',
      guide:'start',about:'status'
    };
    const helpHref=`./guide.html#${guideAnchors[pageKey]||'start'}`;
    ...
    style.textContent=`
      ...
      .ul-menu,.ul-back,.ul-help{flex:0 0 auto;width:34px;height:34px;padding:0;border:1px solid #2c3558;border-radius:9px;background:#11172b;color:#fff;cursor:pointer;text-decoration:none;display:grid;place-items:center;font-weight:850}
      .ul-menu{display:none;font-size:18px}.ul-back{font-size:17px}.ul-help{font-size:15px;color:#9edcff}
      .ul-menu:hover,.ul-back:hover,.ul-help:hover,.ul-menu:focus-visible,.ul-back:focus-visible,.ul-help:focus-visible{border-color:var(--ul-accent);background:#171f39}
      ...
      .ul-drawer{position:absolute;left:0;top:0;bottom:0;width:min(310px,86vw);padding:calc(env(safe-area-inset-top) + 13px) 11px 18px;background:#090d1b;border-right:1px solid #30395c;box-shadow:20px 0 60px #000b;transform:translateX(-105%);transition:transform .2s ease;overflow-y:auto}
      ...
      .ul-drawer-help{margin-top:10px!important;border-color:#385273!important;background:#0c1b2c!important;color:#bce6ff!important}
      ...
      @media(max-width:390px){
        .ul-menu,.ul-back,.ul-help{width:33px;height:33px}
    ...
    nav.innerHTML=`
      ...
      <a class="ul-help" href="${helpHref}" aria-label="Hilfe zu ${currentLabel}" title="Hilfe zu ${currentLabel}">?</a>
    ...
    overlay.innerHTML=`
      <aside class="ul-drawer" aria-label="UniverseLab Seitenmenü" role="dialog" aria-modal="true">
        ...
        ${links}
        <a class="ul-drawer-help" href="${helpHref}"><span>?</span><span>Hilfe zu ${currentLabel}</span></a>
